...
\item Select metric covariate to be displayed as termplot using the dialog (see figure \ref{fig-001}). Note that within the IDE \textbf{RStudio} the selection dialog appears as text list in the integrated console. 

\begin{figure}[ht]
\centering 
\includegraphics[height=3cm]{fig-001} 
...
\paragraph{Preselect metric covariate and plot type}
...
For the function \texttt{lm.interactive()} the values \texttt{"effect"} (the default) and \texttt{"marginal"} are valid. 
Preselecting the variable and the plot type is beneficial  when used in conjunction with the argument \texttt{initial.values} and the automatic save functionality to reproduce plots without user interaction, see the example in the last paragraph of this section.

\paragraph{Select first group only}
If a model contains many groups it may be desirable to select only a subset of groups to be displayed. To allow for a positive selection in this case all groups except the first can be deselected at the beginning:

\begin{small}
\begin{verbatim}
glm.interactive(model.2.fac, selecet.all.groups.begin = FALSE)
\end{verbatim}
\end{small}
...
arguments \texttt{initial.values}, \texttt{preselect.var} and \texttt{preselect.type} this can be used to reproduce the plots without user interaction. In case of the example model with 2 factors the plot of the marginal effects with a prespecified covariate constellation is saved to the actual working directory by
...
\noindent For the function to work the characters may neither appear within the factor nor the level names,
see the documentation of \texttt{factor.combinations()}.
...
Many graphical elements can be directly controlled by arguments in the function call. The major appearance of the plots can be controlled by manipulation of \texttt{par()}-arguments. If necessary graphical elements can be added to the plots by low-level plotting commands in the \textsf{R Console}. Note that elements added this way are not captured by the autosave functionality described at the end of section~\ref{sec-usage}. The example B1 in appendix~\ref{sec-app-addexample} shows how plots can nevertheless be saved without user interaction.
...
\noindent The levels of the additional factor \enquote{job} are represented by the linetype here. If a model contains 4 or more factors it may be advisable to display only a subset of groups by deselecting some of these in the checkbox. The visual discrimination of 4 or more factor will be conceptually hard to achieve.
...
If more detailed control over the rug is needed, the rug needs to be suppressed and added  with \texttt{rug()} from the \textsf{R Console}. The example B1 in section~\ref{sec-app-addexample} shows how to add a customized rug plot with transparent colors.
...
\noindent Note that for a really large number of groups not every group can be seen in the GUI-panel because the panel grows beyond the screen margin. In former versions of \textbf{rpanel} (< 1.1-3) the entries of the checkbox were squeezed together and currently there is no way to circumvent this problem. 
...
\end{enumerate}
Of course the four prerequisites stated in section~\ref{sec-quickstart} must be met, too. For instance a fitted model object obtained by \texttt{gam()} from the package \textbf{gam} \citep{Hast2013} is compatible:
...
\section{Additional example} \label{sec-app-addexample}

\paragraph{Example B1} A customized rug plot with transparent color is added by means of the low-level plotting command \texttt{segments()} and the result is saved to the current working directory.
